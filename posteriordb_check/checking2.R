...
## A table of output per program with median and min and max ##
N_models = 0
model_record = c()
for(l in 1:L_pn){
  modelname <- pn[l]
...
p_box_iter <- ggplot(df, aes(y = model, x = n_iters)) + geom_boxplot() + 
...
jpeg(filename = paste0("../pics/box_leapfrogs_log.jpeg"),
     width = width*1.3, height = height*2, units = "px", pointsize = 12)
p_box_leapfrog <- ggplot(df, aes(y = model, x = n_leapfrogs)) + 
