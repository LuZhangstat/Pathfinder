...
#   w_d_ADVI_mfc <- wasserstein(a_ADVI_mfc, b, p = 1) #wasserstein(a_ADVI_mfc, b, p = 2); w_d_ADVI_mfc
#   
#   # ADVI: fullrank #
#   a_ADVI_fr = wpp(t(ADVI_fullrank_draw[[i]]),
#                   mass = rep(1 / ncol(ADVI_fullrank_draw[[i]]),
#                              ncol(ADVI_fullrank_draw[[i]])))
#   w_d_ADVI_fr <- wasserstein(a_ADVI_fr, b, p = 1) #wasserstein(a_ADVI_fr, b, p = 2); w_d_ADVI_fr
#   
#   
...
# 
# save(file = "../results/wasserstein_phI_adapt_default_RVI.RData",
#      list = c("w_d_matrix"))
# 
# ## check the output ##
# load("../results/wasserstein_phI_adapt_default_RVI.RData") #default; long_hist, short_L, large_K
# colMeans(w_d_matrix)
...
# 
# setEPS()
# postscript("../pics/phI_adapt_default/W_d_compar_100_each_W1.eps",  #_resam_each
#            width = width, height = height)
# print(p_w_d_compar)
# dev.off()

## compute the wasserstein distance for Stan phase I sampler ##
w_d_phI <- c()
t_0 <- proc.time()
for(i in 1:49){ #length(model_record)
  modelname <- pn[model_record[i]]
  ...
  a_phI = wpp(PhaseI_last_draw[[i]],
               mass = rep(1 / nrow(PhaseI_last_draw[[i]]),
                          nrow(PhaseI_last_draw[[i]])))
  b = wpp(ref_samples, mass = rep(1 / nrow(ref_samples), nrow(ref_samples)))
  w_d_phI[i] <- wasserstein(a_phI, b, p = 1) #wasserstein(a_phI, b, p = 2); w_d_phI
}
proc.time() - t_0
# save(file = "../results/wasserstein_phI.RData",
#      list = c("w_d_phI"))
...
for(i in 1:49){ #length(model_record)
  ...
  for(j in 1:M){
    ...
    w_d_pf_M <- wasserstein(a_M, b, p = 1); w_d_pf_M
    ...
    #    (i == 46 && j == 95)){
      ...
      w_d_ADVI_mf <- wasserstein(a_ADVI_mf, b, p = 1); w_d_ADVI_mf
    ...
    w_d_ADVI_fr <- wasserstein(a_ADVI_fr, b, p = 1); w_d_ADVI_fr

    W_d_100_ADVI_fr[j, i] <- w_d_ADVI_fr

    cat("pf:", W_d_100_pf[j, i], "\t", "multi pf:", W_d_100_pf_IR[j, i], "\t",
...
for(i in 1:49){
  modelname <- pn[model_record[i]]
  printf("model %d: %s", model_record[i], modelname)
  print(cbind(pf_w_d_qtils[, i], pf_M_w_d_qtils[, i], pf_w_d_qtils_2[, i], 
...
for(i in 1:49){ #length(model_record)
  ...
  for(j in 1:M){
    ...
    w_d_pf2 <- wasserstein(a, b, p = 1); w_d_pf2
    ...
    w_d_pf3 <- wasserstein(a, b, p = 1); w_d_pf3
...
save(file = "../results/wasserstein_100_sen_W1.RData",
...
for(i in 1:49){ #length(model_record)
  ...
  for(j in 1:M){
    ...
    w_d_pf_I5 <- wasserstein(a_M, b, p = 1); w_d_pf_I5
    ...
    w_d_pf_I40 <- wasserstein(a_M, b, p = 1); w_d_pf_I40
...
# save(file = "../results/wasserstein_100_sen_I_W1_WR.RData",
...
## compute the ELBO for single-path Pathfinder ##
...
load(file = "../results/lp_posteriordb_phI_adapt_default.RData")

ELBO_pf = array(data = NA, dim = c(M, length(model_record)))
...
for(i in 1:49){ #length(model_record)
  ...
  # the log-density function in the unconstrained space #
  lp <- function(theta) log_prob(posterior, theta, adjust_transform = TRUE, 
                                 gradient = TRUE)[1] 
  
  for(j in 1:M){
    if(lp_opath[[i]]$opath[[j]]$status == "mode"){
      ELBO_pf[j, i] = NA
    }else{
      lp_pf <- apply(lp_opath[[i]]$opath[[j]]$DIV_save$repeat_draws[-(D+1), ], 
                     2, lp)
      ELBO_pf[j, i] = mean(lp_pf - 
                             lp_opath[[i]]$opath[[j]]$DIV_save$lp_approx_draws)
    }
  }
}
proc.time() - t_0

save(file = "../results/ELBO_pf.RData",
     list = c("ELBO_pf"))
