...
# parameters settings #
alpha = 0.01
N = 24    # Maximum iters in optimization
...
init_param_unc <- opath[[1]][1, -ncol(opath[[1]])]
...
for(i in 1:length(model_record)){
  ...
  p_lp <- ggplot(data = p_lp_trace, 
    ...
    geom_line(colour = "grey") + geom_point(size = 3) + 
    geom_text(aes(label = iter), vjust = -0.8, size = 5, color = "black") +
  ...
  p_lp_trace_2 = 
    data.frame(
      lp_paths <- unlist(lp_opath[[i]]$pick_records[[1]]$record_lp_draws),
      path_iter <- unlist(lapply(lp_opath[[i]]$pick_records[[1]]$record_lp_draws,
                                 f <- function(x){
                                   lapply(x, g <- function(g){1:length(g)})})),
      point_id <- 
        unlist(lapply(1:length(lp_opath[[i]]$pick_records[[1]]$record_lp_draws),
                      f <- function(x){
                        rep(x, length(unlist(lp_opath[[i]]$pick_records[[1]]$
                                            record_lp_draws[[x]])))})),
      path_id <- unlist(
        lapply(lp_opath[[i]]$pick_records[[1]]$record_lp_draws, 
                               f <- function(x){
                                 lapply(1:length(x), 
                                        g <- function(y){rep(y, length(x[[y]]))
                                          })})),
      stepsize <- unlist(
        lapply(1:length(lp_opath[[i]]$pick_records[[1]]$record_lp_draws),
               f <- function(x){
                 rep(lp_opath[[i]]$pick_records[[1]]$record_stepsize[[x]],
                     sum(unlist(lapply(
                       lp_opath[[i]]$pick_records[[1]]$record_lp_draws[[x]], 
                            length))))})))
  ...
  p_lp_2 <- ggplot(data = p_lp_trace_2, 
                 aes(x=path_iter, y=lp_paths, group=path_id)) +
    facet_wrap(~point_id, scales = "free_y") + 
    geom_line(colour = "grey") + geom_point(size = 1.0) +
  ...
  lay <- rbind(c(1,1,1,1,1),
               ...
               c(2,2,2,2,2),
               c(2,2,2,2,2))
  grid.arrange(p_lp, p_lp_2, layout_matrix = lay)
  dev.off()
