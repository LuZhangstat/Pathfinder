...
library(ggplot2)
library(cmdstanr)
set_cmdstan_path("/home/luzhang/Google Drive/Github/adaptation/cmdstan-2.26.1") # specify the path of cmdstan 
library(loo)
...
ELBO_ADVI_mf = array(data = NA, dim = c(M, length(model_record)))
...
for(i in 1:length(model_record)){
  ...
  while(j <= M){
    ...
    if(ADVI_work){
      ...
      # record ELBO for RVI#
      lp_recal <- apply(unconstrained_draws, 1, lp)
      ELBO_ADVI_mf[j, i] = mean(lp_recal - fit_ADVI$lp_approx())
...
ELBO_ADVI_fr = array(data = NA, dim = c(M, length(model_record)))
iter_ADVI_fr <- array(data = 0, dim = c(M, length(model_record)))
...
for(i in 1:length(model_record)){
  ...
  while(j <= M){
    ...
    if(ADVI_work){
      ...
      # record ELBO for ADVI#
      lp_recal <- apply(unconstrained_draws, 1, lp)
      ELBO_ADVI_fr[j, i] = mean(lp_recal - fit_ADVI$lp_approx())
      
      
      latent_dyn <- read.csv(fit_ADVI$latent_dynamics_files())
      iter_fit <- as.numeric(latent_dyn$X..stan_version_major...2[
        length(latent_dyn$X..stan_version_major...2)-2])
      iter_ADVI_fr[j, i] <- iter_fit
      eta_i = which(eta_sequence == fit_ADVI$metadata()$eta)
...
save(file = "../results/ADVI_100_updat.RData",
     list = c("ADVI_meanfield_draw", "ADVI_meanfield_center", 
              ...
              "ELBO_ADVI_mf", "ELBO_ADVI_fr",
              "iter_ADVI_mf", "iter_ADVI_fr"))
