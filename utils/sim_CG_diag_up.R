...
opt_path <- function(init, fn, gr, lp_f, 
  ...
  for (j in 1:N_mode_max){
    ...
    G_ind = which(splited_output == "parameters")
    Iter = length(G_ind) - 1;
    X = matrix(NA, nrow = Iter, ncol = D)
    G = matrix(NA, nrow = Iter, ncol = D)
    for(g in 1:Iter){
      x = as.numeric(
        splited_output[G_ind[g]:(G_ind[g+1]-1)])
      X[g, ] = x[!is.na(x)][1:D]
      G[g, ] = gr(X[g, ])
    }
    ...
    Ykt = G[2:center_ind, ] - G[1:(center_ind-1), ]
    Skt = X[2:center_ind, ] - X[1:(center_ind-1), ]
    Dk = c()
    thetak = c()
    m = center_ind - 1
    for(i in 1:m){
    ...
    if(any(Dk < 0 | abs(thetak) > 1e12 | is.na(thetak))){   #any(Dk < 0 | abs(thetak) > 1e7)
      print("Negative or unstable Hessian")
      Neg_D_ind = which(Dk < 0 | abs(thetak) > 1e12 | is.na(thetak))
      Ykt = Ykt[-Neg_D_ind, ]
      Skt = Skt[-Neg_D_ind, ]
      Dk = Dk[-Neg_D_ind]
      ...
      m = min(Iter - 1, m)
      m = max(m, 2)
      x_center = X[((1:center_ind)[-(Neg_D_ind + 1)]), ][length(fn_ls_copy), ]
    ...
    if(m >= 10){ # if m >= 10 # m > lskupn
      ...
      if(length(small_lsk_ind) > 0){
        Ykt = Ykt[-small_lsk_ind, ]
        Skt = Skt[-small_lsk_ind, ]
        Dk = Dk[-small_lsk_ind]
    ...
    if( 2*m >= D){
      ...
      Hk = diag(1 / theta_D) + crossprod(Ykt%*%diag(1/(theta_D)), ninvRST) + 
        crossprod(ninvRST, Ykt%*%diag(1/(theta_D)))  + 
        crossprod(ninvRST, (diag(Dk) + tcrossprod(Ykt%*%diag(1/sqrt(theta_D)))) %*% ninvRST)
      cholHk = chol(Hk)
